class BarChart {
  constructor(obj) {
    ...
    //Scalers
    this.scalerNormalBars =
      this.chartHeight / max(this.data.map((row) => row[this.yValues[0]]));
    this.scalerBars100 = this.chartHeight / 100;

    // Add max values of each value to get the max sum and consequential scaler value
    if (this.chartType === "stacked" || this.chartType === "100") {
      let maxSum = max(
        this.data.map((row) => row[this.yValues[0]] + row[this.yValues[1]])
      );
      this.scalerStackedBars = this.chartHeight / maxSum;
    }
    // Make the scaler scale bars to 80% of the width
    ...
    this.titleYOffset =
      obj.titleYOffset || this.chartHeight + this.chartHeight / 10;
  ...
  renderLabels() {
    ...
    for (let i = 0; i <= this.numTicks; i++) {
      ...
      // Handle rendering for population pyramid chart type
        ...
        let maxPop = max(this.data.map((row) => row[this.yValues[0]] + row[this.yValues[1]]));
        ...
        for (let i = 0; i < labels.length; i++) {
          ...
          let formattedLabel =
            labels[i] === 0 ? "0" : (Math.abs(labels[i]) / 1000000).toFixed(0) + "M";
      ...
      for (let i = 0; i < this.data.length; i++) {
        ...
        if (
          ...
          textAlign(CENTER);
          textSize(this.axisLabelTextSize);
          // Move the axis label to the left and rotate it to fit
          translate(-this.chartWidth / 4, -this.chartHeight / 2);
          rotate(this.axisLabelRotation);
          fill(this.axisLabelColour);
          text(this.axisLabelName2, 0, 0); // Render the axis label
          pop();
        } else {
          push();
          // Position the label in the middle of the bar and adjust it
          textSize(this.dataLabelTextSize);
          fill(this.dataLabelColour);

          push();
          translate(xPos + this.barWidth / 2, 10);
          rotate(this.verticalBarDataLabelRotation); // Rotate the label for vertical bars
          ...
          textAlign(CENTER);
          textSize(this.axisLabelTextSize);
          // Position the axis label at the bottom center of the chart
